...
def calc_MC(results) -> np.floating:
    ...
    #x_mean = np.mean(np.array([x for x,y in results], dtype=object))
    y_mean = np.mean(np.array([y for x,y in results]))
    #y_mean = np.mean(np.array([y for x,y in results], dtype=object))
...
def evaluate(model, dataloader, is_classification=False, data="SP500", af="relu", n_hid=5, n_seq=20):
    model.eval()
    l1 = torch.nn.L1Loss()
    l2 = torch.nn.MSELoss()
    l1_losses = []
...
#    file_name = "./results/"+data+"_"+af+"_"+str(n_hid)+"_"+str(n_seq)+".txt" 
#    with open(file_name, mode='w') as fw: 
#        fw.write(f"Validation l1_loss = {np.array(l1_losses).mean()}, l2_loss = {np.array(l2_losses).mean()}, mc = {mc}")
#        fw.write("\n") 
